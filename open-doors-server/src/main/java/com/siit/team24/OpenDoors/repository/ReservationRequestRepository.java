...
public interface ReservationRequestRepository extends JpaRepository<ReservationRequest, Long> {
    ...
    List<ReservationRequest> getPastForGuestAndHostConfirmed(String guestId, String hostId);
    ...
    List<ReservationRequest> getPastForGuestAndAccommodationConfirmed(String guestId, Long hostId, Timestamp deadline);
    ...
    @Query("select distinct r.guest.id from ReservationRequest r where r.accommodation.host.id = ?1 and r.id not in ?2 and r.status = 1 " +
            "and r.dateRange.endDate < current_timestamp")
    List<String> getGuestsByHostId(String hostId, List<Long> evidencedReservationIds);
    ...
    @Query("select distinct r.accommodation.host.id from ReservationRequest r where r.guest.id = ?1 and r.id not in ?2 and r.status = 1 " +
            "and r.dateRange.endDate < current_timestamp")
    List<String> getHostsByGuestId(String guestId, List<Long> evidencedReservationIds);

    @Query("select r from ReservationRequest r where r.accommodation.host.id = ?1 and r.guest.id = ?2")
    List<ReservationRequest> findByHostAndGuest(String hostId, String guestId);
}
